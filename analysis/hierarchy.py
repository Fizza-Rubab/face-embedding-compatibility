...
# ======================================================
# ANALYSIS 4: TRANSITIVITY CHECK
# ======================================================

def analyze_transitivity(matrix, models, threshold=70, save_path=None):
    """
    Test if A→B and B→C good alignment implies A→C good alignment.
    """
    n = len(models)
    transitive_scores = []
        ...
        for j in range(n):
            for k in range(n):
                if i != j and j != k and i != k:
                    ...
                    # Only consider cases where both intermediate alignments are good
                    if (not np.isnan(R_ij) and not np.isnan(R_jk) and not np.isnan(R_ik) and
                        R_ij > threshold and R_jk > threshold):
                        
                        expected = (R_ij + R_jk) / 2
                        transitive_scores.append({
                            'Path': f"{models[i]}→{models[j]}→{models[k]}",
                            'i→j': R_ij,
                            'j→k': R_jk,
                            'i→k (actual)': R_ik,
                            'i→k (expected)': expected,
                            'Difference': R_ik - expected,
                            'Transitivity': 'Good' if R_ik >= expected else 'Poor'
                        })
    
    if len(transitive_scores) == 0:
        print(f"\n⚠ No transitive paths found with threshold={threshold}%")
        return None
    
    df_trans = pd.DataFrame(transitive_scores).sort_values('Difference', ascending=False)
    
    # Save to CSV
    csv_path = f"{CONFIG['output_dir']}/transitivity_analysis.csv"
    df_trans.to_csv(csv_path, index=False)
    print(f"\n✓ Transitivity analysis saved to: {csv_path}")
    
    print(f"\nTransitivity Analysis (threshold={threshold}%):")
    print(f"Mean transitivity gap: {df_trans['Difference'].mean():.2f}%")
    print(f"Positive cases (better than expected): {(df_trans['Difference'] > 0).sum()}")
    print(f"Negative cases (worse than expected): {(df_trans['Difference'] < 0).sum()}")
    
    print(f"\nTop 5 Positive Deviations (better than expected):")
    print(df_trans.head(5)[['Path', 'i→k (actual)', 'i→k (expected)', 'Difference']].to_string(index=False))
    
    print(f"\nTop 5 Negative Deviations (worse than expected):")
    print(df_trans.tail(5)[['Path', 'i→k (actual)', 'i→k (expected)', 'Difference']].to_string(index=False))
    
    if save_path:
        # Visualization
        fig, axes = plt.subplots(1, 2, figsize=(14, 5))
        
        # Left: Histogram of transitivity gaps
        ax = axes[0]
        ax.hist(df_trans['Difference'], bins=30, alpha=0.7, color='steelblue', edgecolor='black')
        ax.axvline(0, color='red', linestyle='--', linewidth=2, label='Perfect transitivity')
        ax.set_xlabel('Transitivity Gap (Actual - Expected) (%)', fontsize=12)
        ax.set_ylabel('Frequency', fontsize=12)
        ax.set_title('Distribution of Transitivity Gaps', fontweight='bold')
        ax.legend()
        ax.grid(alpha=0.3)
        
        # Right: Actual vs Expected scatter
        ax = axes[1]
        ax.scatter(df_trans['i→k (expected)'], df_trans['i→k (actual)'], 
                  s=50, alpha=0.6, c=df_trans['Difference'], cmap='RdYlGn')
        
        # Perfect transitivity line
        lims = [
            np.min([ax.get_xlim(), ax.get_ylim()]),
            np.max([ax.get_xlim(), ax.get_ylim()])
        ]
        ax.plot(lims, lims, 'k--', alpha=0.5, linewidth=2, label='Perfect transitivity')
        
        ax.set_xlabel('Expected Accuracy (avg of i→j and j→k) (%)', fontsize=12)
        ax.set_ylabel('Actual Accuracy i→k (%)', fontsize=12)
        ax.set_title('Transitivity: Expected vs Actual', fontweight='bold')
        ax.legend()
        ax.grid(alpha=0.3)
...
df_trans = analyze_transitivity(
    rank1_matrix,
    CONFIG['models'],
    threshold=70,
    save_path=f"{CONFIG['output_dir']}/transitivity.svg"
)
