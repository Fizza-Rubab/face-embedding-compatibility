...
def save_verification_results(all_results, aggregated_results, modelX, modelY, config):
    ...
    # Save ROC curves
    roc_path = f"{out_dir}/{prefix}_roc_curves.svg"
    plot_roc_curves_aggregated(all_results, aggregated_results,
                               modelX, modelY, roc_path)


def plot_roc_curves_aggregated(all_results, aggregated_results, modelX, modelY, save_path):
    """
    Plot ROC curves with mean and confidence intervals.
    Saves as SVG for publication quality.
    """
    import matplotlib.pyplot as plt
    
    method_names = list(aggregated_results.keys())
    
    # We need to extract FMR/TMR curves from individual seed results
    # However, these aren't stored in aggregated_results, so we need to recompute
    # For simplicity, we'll plot using the first seed's data
    # (In practice, you'd want to interpolate and average across seeds)
    
    fig, ax = plt.subplots(figsize=(8, 6))
    
    colors = plt.cm.tab10(np.linspace(0, 1, len(method_names)))
    
    # Since we don't have full ROC curves in results, we'll plot points at key FMR values
    # This is a simplified version - for full curves, you'd need to save FMR/TMR arrays
    
    for i, method_name in enumerate(method_names):
        if method_name not in aggregated_results:
            continue
        
        metrics = aggregated_results[method_name]
        
        # Get mean values
        auc_mean = metrics['auc']['mean']
        tmr_001_mean = metrics['tmr_at_fmr_0.01']['mean'] / 100  # Convert to 0-1
        tmr_01_mean = metrics['tmr_at_fmr_0.1']['mean'] / 100
        tmr_1_mean = metrics['tmr_at_fmr_1']['mean'] / 100
        tmr_10_mean = metrics['tmr_at_fmr_10']['mean'] / 100
        
        # Plot key points
        fmr_points = [0.0001, 0.001, 0.01, 0.1]
        tmr_points = [tmr_001_mean, tmr_01_mean, tmr_1_mean, tmr_10_mean]
        
        linestyle = '--' if method_name == 'Baseline' else '-'
        linewidth = 2.5 if method_name == 'Baseline' else 2.0
        markersize = 8 if method_name == 'Baseline' else 6
        
        ax.plot(fmr_points, tmr_points, 
               linestyle=linestyle, 
               linewidth=linewidth,
               marker='o',
               markersize=markersize,
               label=f'{method_name} (AUC={auc_mean:.3f})',
               color=colors[i],
               alpha=0.8)
    
    # Diagonal reference line (random performance)
    ax.plot([0.0001, 1], [0.0001, 1], 'k--', linewidth=1, alpha=0.3, label='Random')
    
    ax.set_xscale('log')
    ax.set_xlabel('False Match Rate (FMR)', fontsize=12)
    ax.set_ylabel('True Match Rate (TMR)', fontsize=12)
    ax.set_title(f'ROC Curves: {modelX} → {modelY}\n(Mean across {len(all_results)} seeds)',
